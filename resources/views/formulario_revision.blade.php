                          ...
                          @foreach($descripcion['LISTA_DISTRIBUCION'] as $distribucion)
                            <tr>
                              ...
                              <td>
                                ...
                                <!--<button class="btn btn-primary pull-right" type="button" onclick="mensaje({{$distribucion->FK_LISTA_DISTRIBUCION}},10)">Mensaje</button>-->
                              </td>
                              <!-- {{$i++}} -->
                            </tr>
                          @endforeach
...
<script type="text/javascript">
 //var ID_DESCRIPCION;
  ...
  $( window ).load(function() {
    ...
    console.log('Formulario revision');
    /*console.log(puestos);
    console.log(ldist);
    console.log(disSob);//*/
    llenado();
...
function guardar_Actividades(tmp_cont_actG,elemento){
  ...
  function guardar_formacion(){
    ...
    function mensaje(idElemento,elemento){
      //swal("", "Favor de no dejar campos vacíos", "warning");
      //console.log(idElemento);
      //console.log(jQuery().jquery);
      
      swal("Ingrese el mensaje:", {
        content: "input",
      })
      .then((value) => {
        if(`${value}` != 'null' && `${value}` != ''){
          ...
          console.log(`${value}`);
          var success;
          var url = "/descripcion/mensaje"
          var dataForm = new FormData();
          dataForm.append('idElemento',idElemento);
          dataForm.append('elemento',elemento);
          dataForm.append('mensaje',`${value}`);
          dataForm.append('id_descripcion',id_des);
          metodoAjax(url,dataForm,function(success){
            swal("", "Mensaje guardado satisfactoriamente", "success");
          });//*/
        }else{
          //swal('Sin mensaje');
        }
